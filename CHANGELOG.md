...
**Spending-limit fixture, and signature shape reporting**
...
- `e2e/contracts/policy-account`, a new contract fixture with unit tests: a
  ...
  over it. (#72)
- `DescribeSignature` and `SignatureShape` report the structure of a signature
  an offline check cannot verify — a 64-byte passkey blob, a classic-account
  signature map, or an unrecognised shape — as a description, and never as a
  verification verdict. `Inspect` reports the shape per credential node.
  (#63)
